        ...
        <div class="max-w-6xl mx-auto space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-5 gap-4">
                <div>
                    ...
                    <h2 class="font-extrabold text-2xl text-slate-800 leading-tight mt-1 flex items-center gap-2.5">
                        <i class="fa-solid fa-shield-halved text-slate-700 text-xl"></i>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                @if($projects->isEmpty())
                    <div class="p-16 text-center">
                        ...
                        <p class="text-xs text-slate-500 max-w-sm mx-auto">{{ __('Belum ada proyek dalam status Planning yang aktif untuk manajemen risiko.') }}</p>
                    </div>
                @else
                                    ...
                                    <tr class="hover:bg-slate-50/30 transition duration-150">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            @elseif($riskStatus === 'draft')
                                                <span class="inline-flex items-center gap-1 py-1 px-2.5 rounded-lg text-[10px] font-extrabold border bg-slate-50 text-slate-600 border-slate-200">
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="inline-flex gap-2">
                                                @if($isHrFinalized)
                                                    @if($riskStatus === 'none')
                                                        @if($isPmo)
                                                            <a href="{{ route('projects.risk-management.create', $project->id) }}" class="inline-flex items-center justify-center px-3.5 py-2 text-xs font-bold text-white bg-[#0B1329] hover:bg-slate-800 rounded-xl shadow-sm transition gap-1.5">
                                                    ...
                                                    @elseif($riskStatus === 'draft')
                                                        @if($isPmo)
                                                            <a href="{{ route('projects.risk-management.edit', $project->id) }}" class="inline-flex items-center justify-center px-3.5 py-2 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl shadow-sm transition gap-1.5">
                                                                <i class="fas fa-edit text-[9px]"></i> {{ __('Kelola Risiko') }}
                                                            </a>
                                                        @else
                                                            <a href="{{ route('projects.risk-management.show', $project->id) }}" class="inline-flex items-center justify-center px-3.5 py-2 text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl shadow-sm transition gap-1.5">
                                                ...
                                                @else
                                                    <span class="text-xs text-slate-400 italic font-semibold py-2 px-3 block flex items-center gap-1">
                                                        <i class="fas fa-lock text-[9px]"></i> {{ __('Menunggu SDM Final') }}
                                                    </span>
                                                @endif
                                            </div>
                                        </td>

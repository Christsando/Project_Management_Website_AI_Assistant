            ...
            <div class="bg-white rounded-xl border border-[#e3e3e0] shadow-sm p-6">
                <form action="{{ route('projects.human-resource.store', $project->id) }}" method="POST">
                    ...
                    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 text-blue-800 rounded-xl text-xs flex items-start gap-2.5">
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-secondaryText leading-relaxed">
                                {{ __('Inisialisasi ini akan membuat draf Perencanaan Sumber Daya Manusia (SDM) baru untuk proyek ini. Setelah diinisialisasi, Anda dapat menambahkan rincian kebutuhan tim pelaksana proyek, mendefinisikan kriteria skill, deskripsi tugas pekerjaan, dan menetapkan penanggung jawab (PIC) langsung terhadap struktur kerja (WBS) yang ada.') }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        <a href="{{ route('project-planning.human-resource.index') }}" class="px-4 py-2 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-xl text-sm font-semibold transition">
                            {{ __('Batal') }}
                        </a>

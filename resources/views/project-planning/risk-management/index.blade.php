            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 border-b border-slate-50 pb-5">
                <div>
                    <h2 class="font-extrabold text-2xl text-slate-800 leading-tight flex items-center gap-2">
                        ...
                        {{ __('Project Risk Management') }}
                    </h2>
                    <p class="text-sm text-slate-500 mt-1">
                        @if(strtolower(Auth::user()->role) === 'pmo' || strtolower(Auth::user()->role) === 'project management officer')
                            {{ __('Identifikasi potensi risiko proyek, tentukan dampak, probabilitas, keparahan, dan rumuskan rencana mitigasi menggunakan AI Assistant.') }}
                        @else
                ...
                <div>
                    <a href="{{ route('project-planning') }}" class="inline-flex items-center justify-center px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 hover:text-slate-900 rounded-xl text-xs font-bold shadow-sm transition gap-1.5">
            ...
            <div class="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
                @if($projects->isEmpty())
                    <div class="p-16 text-center">
                        <div class="w-16 h-16 bg-blue-50 text-blue-505 border border-blue-100/50 rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm">
                            <i class="fa-solid fa-shield-halved text-2xl text-blue-600"></i>
                        </div>
                        <h4 class="font-bold text-slate-800 mb-1">{{ __('Tidak ada proyek ditemukan') }}</h4>
                        <p class="text-xs text-slate-500 max-w-sm mx-auto">{{ __('Belum ada proyek dalam status Planning yang tersedia untuk Anda.') }}</p>
                    </div>
                @else
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                                    ...
                                    <th class="px-6 py-4">{{ __('Status Manajemen Risiko') }}</th>
                            ...
                            <tbody class="divide-y divide-slate-50 text-xs">
                                @foreach($projects as $project)
                                    @php
                                        ...
                                        // HR Plan status
                                        $isHrFinalized = ($project->humanResourcePlan && $project->humanResourcePlan->status === 'finalized');
                                        
                                        // Risk plan status
                                        $riskPlan = $project->riskPlan;
                                        $riskStatus = $riskPlan ? $riskPlan->status : 'none';
                                    @endphp
                                        ...
                                        <td class="px-6 py-4">
                                            @if(!$isHrFinalized)
                                                <span class="inline-flex items-center gap-1 py-0.5 px-2 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200">
                                                    ...
                                                    {{ __('Menunggu SDM Final') }}
                                                </span>
                                            @elseif($riskStatus === 'none')
                                                <span class="inline-flex items-center gap-1 py-0.5 px-2 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
                                                    <span class="w-1 h-1 rounded-full bg-rose-500"></span>
                                                    {{ __('Belum Dibuat') }}
                                                </span>
                                            @elseif($riskStatus === 'draft')
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="inline-flex gap-2">
                                                @if($isHrFinalized)
                                                    @if($riskStatus === 'none')
                                                        @if($isPmo)
                                                            <a href="{{ route('projects.risk-management.create', $project->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-50 border border-blue-100 hover:bg-blue-600 hover:text-white rounded-xl shadow-sm transition gap-1">
                                                                <i class="fas fa-plus text-[9px]"></i> {{ __('Buat Risk Plan') }}
                                                            </a>
                                                        @else
                                                    ...
                                                    @elseif($riskStatus === 'draft')
                                                        @if($isPmo)
                                                            <a href="{{ route('projects.risk-management.edit', $project->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-amber-700 bg-amber-50 border border-amber-100 hover:bg-amber-600 hover:text-white rounded-xl shadow-sm transition gap-1">
                                                                <i class="fas fa-edit text-[9px]"></i> {{ __('Kelola Risiko') }}
                                                            </a>
                                                        @else
                                                            <a href="{{ route('projects.risk-management.show', $project->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl shadow-sm transition gap-1">
                                                                <i class="fas fa-eye text-[9px] text-slate-400"></i> {{ __('Detail Risiko') }}
                                                            </a>
                                                        @endif
                                                    @else
                                                        <a href="{{ route('projects.risk-management.show', $project->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl shadow-sm transition gap-1">

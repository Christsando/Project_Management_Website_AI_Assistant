    ...
    <div class="pl-4 pt-4">
        <div class="bg-cardSection rounded-xl p-6 max-w-3xl mx-auto">
            ...
            <div class="bg-white p-6 rounded-xl border border-[#e3e3e0] shadow-sm">
                <form action="{{ route('projects.wbs.update', [$project->id, $wbsItem->id]) }}" method="POST" id="wbsForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4" 
                                      ...
                                      placeholder="Jelaskan secara detail apa yang akan dikerjakan pada tugas ini...">{{ old('description', $wbsItem->description) }}</textarea>
                            @error('description')
                                <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="priority" id="priority" 
                                        class="w-full px-4 py-2.5 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition">
                                    <option value="low" {{ old('priority', $wbsItem->priority) === 'low' ? 'selected' : '' }}>{{ __('Low') }}</option>
                                    <option value="medium" {{ old('priority', $wbsItem->priority) === 'medium' ? 'selected' : '' }}>{{ __('Medium') }}</option>
                                    <option value="high" {{ old('priority', $wbsItem->priority) === 'high' ? 'selected' : '' }}>{{ __('High') }}</option>
                                </select>
                                @error('priority')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror

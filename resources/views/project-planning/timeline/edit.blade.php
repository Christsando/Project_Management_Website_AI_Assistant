    ...
    <div class="pl-4 pt-4">
        <div class="bg-cardSection rounded-xl p-6 max-w-3xl mx-auto">
            ...
            <div class="bg-white p-6 rounded-xl border border-[#e3e3e0] shadow-sm">
                <form action="{{ route('projects.timeline.update', [$project->id, $timelineItem->id]) }}" method="POST" id="timelineForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="wbs_item_id" id="wbs_item_id" 
                                ...
                                @foreach($wbsItems as $wbs)
                                    <option value="{{ $wbs->id }}" {{ old('wbs_item_id', $timelineItem->wbs_item_id) == $wbs->id ? 'selected' : '' }}>
                                        {{ $wbs->title }} (ID: #{{ $wbs->id }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $timelineItem->start_date ? $timelineItem->start_date->format('Y-m-d') : '') }}"
                                       class="w-full px-4 py-2.5 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition">
                                @error('start_date')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $timelineItem->end_date ? $timelineItem->end_date->format('Y-m-d') : '') }}"
                                       class="w-full px-4 py-2.5 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition">
                                @error('end_date')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            <select name="dependency_wbs_item_id" id="dependency_wbs_item_id" 
                                ...
                                @foreach($dependencyItems as $dep)
                                    <option value="{{ $dep->id }}" {{ old('dependency_wbs_item_id', $timelineItem->dependency_wbs_item_id) == $dep->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="is_milestone" id="is_milestone" 
                                    class="w-full px-4 py-2.5 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition">
                                <option value="0" {{ old('is_milestone', $timelineItem->is_milestone ? '1' : '0') === '0' ? 'selected' : '' }}>{{ __('Bukan Milestone') }}</option>
                                <option value="1" {{ old('is_milestone', $timelineItem->is_milestone ? '1' : '0') === '1' ? 'selected' : '' }}>{{ __('Ya, ini Milestone') }}</option>
                            </select>
                            @error('is_milestone')
                                <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="milestone_name_group" class="{{ old('is_milestone', $timelineItem->is_milestone ? '1' : '0') === '1' ? '' : 'hidden' }}">
                            <label for="milestone_name" class="block text-sm font-semibold text-primaryText mb-1.5">{{ __('Nama Milestone *') }}</label>
                            <input type="text" name="milestone_name" id="milestone_name" value="{{ old('milestone_name', $timelineItem->milestone_name) }}"
                                   class="w-full px-4 py-2.5 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                   placeholder="Tuliskan nama pencapaian utama/milestone...">
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3" 
                                      ...
                                      placeholder="Catatan tambahan...">{{ old('notes', $timelineItem->notes) }}</textarea>
                            @error('notes')
                                <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

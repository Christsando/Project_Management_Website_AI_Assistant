                ...
                <form action="{{ route('projects.scope.store', $project->id) }}" method="POST" id="scopeForm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="in_scope" id="in_scope" rows="5" 
                                          ...
                                          placeholder="Tuliskan daftar modul, fitur, atau pekerjaan yang masuk dalam proyek (pisahkan dengan baris baru atau poin)...">{{ old('in_scope') }}</textarea>
                                @error('in_scope')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <textarea name="out_of_scope" id="out_of_scope" rows="5" 
                                          ...
                                          placeholder="Tuliskan apa saja yang tidak akan dikerjakan dalam proyek ini agar tidak terjadi scope creep...">{{ old('out_of_scope') }}</textarea>
                                @error('out_of_scope')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="deliverables" id="deliverables" rows="4" 
                                          class="w-full px-4 py-2 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                          placeholder="Sebutkan hasil fisik/non-fisik yang harus diserahkan (misal: dokumen, source code, laporan)...">{{ old('deliverables') }}</textarea>
                                @error('deliverables')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <textarea name="acceptance_criteria" id="acceptance_criteria" rows="4" 
                                          class="w-full px-4 py-2 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                          placeholder="Sebutkan standar atau syarat yang harus dipenuhi agar hasil kerja dapat diterima oleh klien/user...">{{ old('acceptance_criteria') }}</textarea>
                    ...
                    <div class="mb-6">
                        <h4 class="text-sm font-bold text-primaryText uppercase tracking-wider mb-4 flex items-center gap-2">
                            ...
                            {{ __('Persyaratan Tambahan, Asumsi, & Batasan') }}
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="main_requirements" id="main_requirements" rows="3" 
                                          class="w-full px-4 py-2 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                          placeholder="Sebutkan kebutuhan fungsional/non-fungsional utama...">{{ old('main_requirements') }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="assumptions" id="assumptions" rows="3" 
                                          class="w-full px-4 py-2 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                          placeholder="Asumsi-asumsi yang dianggap benar selama siklus perencanaan proyek...">{{ old('assumptions') }}</textarea>
                                @error('assumptions')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <textarea name="constraints" id="constraints" rows="3" 
                                          class="w-full px-4 py-2 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                          placeholder="Faktor pembatas (misal: budget, resource, regulasi, waktu)...">{{ old('constraints') }}</textarea>
                                @error('constraints')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <textarea name="notes" id="notes" rows="3" 
                                          class="w-full px-4 py-2 border border-[#e3e3e0] rounded-xl text-sm text-primaryText focus:ring-primary focus:border-primary transition" 
                                          placeholder="Catatan tambahan penting lainnya terkait ruang lingkup proyek...">{{ old('notes') }}</textarea>
                                @error('notes')
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100 mt-6">
                        <a href="{{ route('projects.show', $project->id) }}" class="px-4 py-2 bg-white border border-gray-300 hover:border-gray-400 text-gray-700 hover:text-gray-900 rounded-xl text-sm font-semibold shadow-sm transition">
                            {{ __('Batal') }}
                        </a>

                ...
                <div>
                    ...
                    <h3 class="text-sm text-secondaryText mt-1">
                        @if(strtolower(Auth::user()->role) === 'manager')
                            {{ __('Kelola rencana anggaran belanja proyek (RAB) berdasarkan timeline yang sudah difinalisasi.') }}
                        @else
                            {{ __('Tinjau rencana anggaran belanja proyek (RAB) dan rincian alokasi biaya.') }}
                        @endif
                    </h3>
                </div>
                <div>
                                            ...
                                            <div class="inline-flex gap-2">
                                                @if($isTimelineFinalized)
                                                    @if($budgetStatus === 'none')
                                                        @if($userRole === 'manager')
                                                            <a href="{{ route('projects.budget.create', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-600 hover:text-white shadow-sm transition">
                                                                <i class="fas fa-plus mr-1.5"></i> {{ __('Buat Budget') }}
                                                            </a>
                                                        @else
                                                    ...
                                                    @elseif($budgetStatus === 'draft')
                                                        @if($userRole === 'manager')
                                                            <a href="{{ route('projects.budget.edit', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg hover:bg-amber-600 hover:text-white shadow-sm transition">
                                                                <i class="fas fa-edit mr-1.5"></i> {{ __('Kelola Budget') }}
                                                            </a>
                                                        @else
                                                            <a href="{{ route('projects.budget.show', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 shadow-sm transition">
                                                                <i class="fas fa-eye mr-1.5 text-gray-400"></i> {{ __('Detail Budget') }}
                                                            </a>
                                                        @endif
                                                    @else
                                                        <a href="{{ route('projects.budget.show', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 shadow-sm transition">

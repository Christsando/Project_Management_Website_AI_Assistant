                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                @foreach($projects as $project)
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-800 border border-blue-200">
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="inline-flex gap-2">
                                                @if($project->scope)
                                                    <a href="{{ route('projects.scope.show', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 shadow-sm transition">
                                                        <i class="fas fa-eye mr-1.5 text-gray-400"></i> {{ __('Detail Scope') }}
                                                    </a>
                                                    @if($project->scope->status === 'draft' && $userRole === 'manager')
                                                        <a href="{{ route('projects.scope.edit', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-300 rounded-lg hover:bg-amber-600 hover:text-white shadow-sm transition">
                                                            <i class="fas fa-edit mr-1.5"></i> {{ __('Edit') }}
                                                        </a>
                                                    @endif
                                                @else
                                                    @if($userRole === 'manager')
                                                        <a href="{{ route('projects.scope.create', $project->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-600 hover:text-white shadow-sm transition">
                                                            <i class="fas fa-plus mr-1.5"></i> {{ __('Buat Scope') }}
                                                        </a>
                                                    @else

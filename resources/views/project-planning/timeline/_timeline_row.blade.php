<tr class="hover:bg-slate-50/30 transition">
    <td class="px-6 py-4">
        <div class="flex items-center min-w-0" style="padding-left: {{ $depth * 20 }}px">
            @if($depth > 0)
                <span class="text-slate-300 mr-2 font-mono shrink-0 select-none">↳</span>
            @endif
            <div class="truncate">
                <span class="font-bold text-slate-800 text-xs md:text-sm block" title="{{ $wbs->title }}">{{ $wbs->title }}</span>
                <span class="text-[9px] text-slate-400 block font-mono mt-0.5">WBS ID: #{{ $wbs->id }}</span>
            </div>
        </div>
    </td>
    <td class="px-6 py-4">
        @if($wbs->timelineItem)
            ...
            @if($wbs->timelineItem->dependencyWbsItem)
                <div class="text-[9px] text-amber-700 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-200/60 inline-flex items-center mt-1 font-bold">
                    <i class="fas fa-link mr-1"></i>
                    Predecessor: #{{ $wbs->timelineItem->dependency_wbs_item_id }} ({{ Str::limit($wbs->timelineItem->dependencyWbsItem->title, 20) }})
                </div>
            @endif
        @else
            <span class="text-[10px] text-rose-700 bg-rose-50 px-2.5 py-0.5 rounded-lg border border-rose-200/50 inline-flex items-center font-bold">
            ...
            @if((strtolower(Auth::user()->role) === 'pmo' || strtolower(Auth::user()->role) === 'project management officer') && !$isTimelineFinalized)
                ...
                @else
                    <a href="{{ route('projects.timeline.create', $project->id) }}?wbs_id={{ $wbs->id }}" 
                       class="inline-flex items-center gap-1 px-3 py-1.5 text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-600 hover:text-white transition shadow-sm">
                        <i class="fas fa-calendar-plus text-[10px]"></i>
                        {{ __('Jadwalkan') }}
                    </a>
                @endif
            @else
                <span class="text-[10px] text-slate-400 italic font-semibold">{{ __('Kunci (Read-only)') }}</span>
            @endif
        </div>
    </td>
</tr>
